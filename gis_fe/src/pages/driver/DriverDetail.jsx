...
const DriverDetail = () => {
    ...
    const fetchPointReview = async () => {
        ...
        if (error) {
            console.log("Lỗi không thể lấy điểm đánh giá trung bình: ", error)
            return
        }
        setPointReview(response.data)
    }
    // console.log(pointReview)
    ...
    return (
        <>
            <Breadcrumb
                routes={[
                    { path: "/driver/booking", name: "Thông tin cá nhân", icon: "profile" },
                ]}
                role='DRIVER'
            />
                ...
                <div className="container mx-auto p-4 ">
                    <div className="grid grid-cols-1 md:grid-cols-[1.5fr_2fr_2fr] gap-4">
                        ...
                        <div className="bg-white p-4 rounded-lg shadow-md col-span-2 border border-blue-400">
                            ...
                            <hr className='my-4' />
                            <div>
                                <DriverRating criteria={pointReview} rating={driver?.star} isAverage="true"></DriverRating>
                            </div>
                        </div>
                    </div>
                            ...
                            <table className="min-w-full border border-gray-200 rounded-lg overflow-hidden shadow">
                                <thead>
                                    <tr className="bg-gray-100">
                                        {["STT", "Thời gian", "Địa điểm", "Khách hàng", "Tổng tiền", "Trạng thái"].map((header) => (
                                            <th
                                                key={header}
                                ...
                                <tbody>
                                    ...
                                    {updatedBookings.length > 0 && updatedBookings.map((booking, index) => (
                                        <tr key={booking.id} onClick={() => navigate(`/driver/booking-detail/${booking.id}`)}
                                    ...
                                    {bookings.length === 0 && (
                                        <tr>
                                            <td colSpan="5" className="text-center italic text-red-400">
                                                Tài xế hiện chưa có chuyến xe nào
                                            </td>
                                        </tr>
                                    )}
