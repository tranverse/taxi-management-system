...
const DriverRevenue = () => {
    ...
    return (
        <>
            ...
            <Breadcrumb
                routes={[
                    { path: "/driver/statistic", name: "Doanh thu", icon: "revenue" },
                ]}
                role="DRIVER"
            />
                        ...
                        >
                            ...
                            <Tooltip
                                formatter={(value, name, props) => {
                                    ...
                                    return [`${props.payload.tripCount.toLocaleString()} chuyến`, "Số chuyến"];
                                }}
                                labelFormatter={(label) => `Ngày: ${label}`}
                                content={({ payload, label }) => {
                                    if (!payload || payload.length === 0) return null;

                                    const revenue = payload.find((item) => item.name === "Doanh thu (VNĐ)")?.value || 0;
                                    const tripCount = payload[0]?.payload.tripCount || 0;

                                    return (
                                        <div className="bg-white p-2  rounded shadow-md">
                                            <p className="text-sm font-bold">Ngày: {label}</p>
                                            <p className="text-blue-500">Doanh thu: {revenue.toLocaleString()} đ</p>
                                            <p className="text-green-500">Số chuyến: {tripCount.toLocaleString()} chuyến</p>
                                        </div>
                                    );
                                }}
                            />
